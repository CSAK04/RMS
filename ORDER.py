...
'''cursor.execute('create table ORDERS(ORDER_NO int primary key,TABLE_NO INT(3),\
                            MCODE int(3), foreign key(MCODE) references MENU(MCODE))')
'''
...
def cancel_order(ORDER_NUMBER):
    cursor.execute('DELETE from orders where ORDER_NO = %s',(ORDER_NUMBER,))
    db.commit()    

#To view all orders of a table
def view_order_details(TABLE_NUMBER):
    cursor.execute('SELECT ITEM,COURSE,PRICE FROM ORDERS O,MENU M WHERE TABLE_NO = %s AND\
